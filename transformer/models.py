...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, dec_inputs_len, enc_inputs, enc_outputs, return_attn=False, is_initial=False):
        dec_outputs = self.tgt_emb(dec_inputs)
        # print('dec_outputs', dec_outputs.size())
        # print('pos', self.pos_emb(dec_inputs_len).size())
...
class Decoder_late(nn.Module):
    def __init__(self, n_layers, d_k, d_v, d_model, d_ff, n_heads, dropout=0.1, weighted=False):
        super(Decoder_late, self).__init__()
        self.d_model = d_model
        # self.tgt_emb = nn.Embedding(tgt_vocab_size, d_model, padding_idx=0, )
        # self.pos_emb = PosEncoding(max_seq_len * 10, d_model) # TODO: *10 fix
        # self.dropout_emb = nn.Dropout(dropout)
    ...
    def forward(self, dec_inputs, dec_inputs_len, dec_out, enc_inputs, enc_outputs, return_attn=False, is_initial=False):
        dec_outputs = dec_out
        # dec_outputs = self.tgt_emb(dec_inputs)
        # print('dec_outputs', dec_outputs.size())
        # print('pos', self.pos_emb(dec_inputs_len).size())
