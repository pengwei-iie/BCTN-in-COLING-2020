...
def metric_max_over_ground_truths(metric_fn, prediction, ground_truths):
    ...
    index = scores_for_ground_truths.index(score)
    return score, index
...
def find_fake_answer(sample):
    ...
    Raises:
        None
    """
    # 下面这个循环是为了得到每一个样例，所有段落中与答案最相关的段落；
    # 并且得到最相关的答案索引，就是说第几个答案最相关，为后面做生成（标准答案）准备
    for doc in sample['documents']:
        ...
        for p_idx, para_tokens in enumerate(doc['segmented_paragraphs']):
            if len(sample['segmented_answers']) > 0:
                related_score, index = metric_max_over_ground_truths(recall,
                                                              para_tokens,
            ...
            if related_score > max_related_score \
                ...
                most_related_para_len = len(para_tokens)
                max_related_score = related_score
                most_answer_index = index
        doc['most_related_para'] = most_related_para
        doc['best_answer_index'] = most_answer_index

    sample['answer_docs'] = []
    sample['answer_spans'] = []
    sample['fake_answers'] = []
    sample['match_scores'] = []
    sample['best_answer_index'] = []
    ...
    for d_idx, doc in enumerate(sample['documents']):
        ...
        for start_tidx in range(len(most_related_para_tokens)):
            ...
            for end_tidx in range(len(most_related_para_tokens) - 1, start_tidx - 1, -1):
                ...
                if len(sample['segmented_answers']) > 0:
                    match_score, index = metric_max_over_ground_truths(f1_score, span_tokens,
                                                                sample['segmented_answers'])
                else:
                    match_score = 0
                    index = 0
    ...
    if best_match_score > 0:
        ...
        sample['best_answer_index'].append(best_answer_index)
...
if __name__ == '__main__':
    ...
    with open('../data/devset/search.dev.json', 'r', encoding='utf-8') as infile:
